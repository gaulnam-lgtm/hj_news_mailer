...
def get_article_image(url: str) -> str | None:
    ...
    try:
        ...
        with urlopen(req, timeout=8) as resp:
            if resp.getcode() != 200: return None
            html = resp.read().decode("utf-8", errors="ignore")

        m = re.search(r'<meta\s+(?:property|name)=["\']og:image["\']\s+content=["\']([^"\']+)["\']', html, re.IGNORECASE)
        if m: return make_absolute_url(url, m.group(1).strip())
...
POLICY_HINTS = ["인앱결제", "외부결제", "제3자결제", "안티스티어링", "사이드로딩", "수수료", "정책", "규제", "법안", "법", "판결", "소송", "심사", "허용", "금지", "강제", "방통위", "공정위", "디지털시장법", "dma", "앱스토어", "플레이스토어", "애플", "구글", "원스토어", "갤럭시스토어"]

def is_relevant_article(keyword, title, desc):
    raw_text = f"{title} {desc}"
    text = normalize_text(raw_text)
    kw = normalize_text(keyword)
    if kw not in text: return False
    for bad in KEYWORDS_EXCLUDE:
        if normalize_text(bad) in text: return False
    if any(sk in kw for sk in STRICT_CONTEXT_KEYWORDS):
        ...
        if not (has_platform or has_policy): return False
    return True
...
def score_article(keyword, title, desc):
    ...
    strong = ["인앱결제", "외부결제", "앱스토어", "플레이스토어", "애플", "구글", "수수료", "정책", "규제", "소송", "방통위", "공정위", "안티스티어링", "사이드로딩", "디지털시장법"]
...
def fetch_naver_articles(keyword):
    ...
    for item in data.get("items", []):
        ...
        try:
            ...
            if pub_dt < week_ago_dt: continue
            pub_label = pub_dt.strftime("%Y.%m.%d")
        except: continue

        if not title or not is_relevant_article(keyword, title, desc): continue
...
def fetch_google_articles(keyword):
    ...
    try:
        ...
        for item in root.findall(".//item")[:10]:
            ...
            if not (title_el and link_el and title_el.text): continue
...
# ── 요약 생성 ───────────────────────────────────────────────
POLICY_KEYWORDS = ["수수료", "정책", "규제", "법", "인하", "허용", "금지", "의무", "심사", "결제", "소송", "방통위", "공정위", "안티스티어링", "외부결제", "인앱결제", "제3자결제", "사이드로딩"]
BAD_STARTS = ["이에 ", "이를 ", "이후 ", "이와 ", "한편 ", "또한 ", "그러나 ", "하지만 ", "따라서 ", "이같은 ", "이번 ", "이런 ", "이같이 "]

def to_bullet_style(text):
    text = clean_spaces(text)
    for b in BAD_STARTS:
        if text.startswith(b): return None
    endings = [("하고 있다", ""), ("되고 있다", ""), ("병행되고 있다", "병행"), ("시행 중이다", "시행"), ("논의 중이다", "논의"), ("검토 중이다", "검토"), ("인하됐으며", "인하"), ("인하됐다", "인하"), ("인하했다", "인하"), ("허용됐다", "허용"), ("허용했다", "허용"), ("도입됐다", "도입"), ("발표됐다", "발표"), ("시행됐다", "시행"), ("강화됐다", "강화"), ("부각됐다", "부각"), ("이어지고 있다", "이어짐"), ("높아지고 있다", "상승"), ("중이다", "중"), ("이다", ""), ("됐다", ""), ("했다", ""), ("한다", ""), ("된다", ""), ("있다", ""), ("없다", ""), ("밝혔다", ""), ("전했다", ""), ("나타났다", ""), ("보인다", ""), ("예정이다", "예정"), ("것이다", ""), ("했으며", ""), ("했고", "")]
...
if __name__ == "__main__":
    ...
    for kw, arts in all_articles.items():
        for art in arts:
            link = art.get("link", "")
            if link: link_to_entries[link].append((kw, art))
    ...
    for kw, remove_links in to_remove.items():
        if kw not in all_articles: continue
